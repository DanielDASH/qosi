package com.queroquest.model;

import java.security.MessageDigest;
import java.sql.*;

import com.queroquest.bean.LoginBean;
import com.queroquest.bean.QuestaoBean;
...
public class Database {
	...
	public boolean inserirAuxTA(QuestaoBean qb) {
		Dao dao = Dao.getDriver();
		String sql = "insert into auxiliar (tipo, areaQuestao) values ";
		...
		try {
			...
			stt.executeUpdate(sql + "('" + qb.getTipo() + "','" + qb.getAreaQuestao() + "');");
	...
	public boolean inserirAuxQAM(QuestaoBean qb) {
		...
		String sql = "select tipo, areaQuestao from auxiliar;";
		...
		try {
			...
			while (rs.next()) {
				...
				aux[1] = rs.getString("areaQuestao");
			}
		...
		try {
			...
			stt = conn.prepareStatement(
					"update auxiliar set enunciado = ?," + " altA = ?," + " altB = ?," + " altC = ?," + " altD = ?,"
							+ " altE = ? where tipo like '" + aux[0] + "' and areaQuestao like '" + aux[1] + "';");
	...
	public void copyTableQAM(String email) {
		...
		String sql = "insert into questao "
				+ " (idUser, tipo, enunciado, altA, altB, altC, altD, altE, areaQuestao) values ";
		...
		try {
			...
			ResultSet rs1 = stt
					.executeQuery("select tipo, enunciado, altA, altB, altC, altD, altE, areaQuestao from auxiliar;");

			while (rs1.next()) {
				aux[1] = rs1.getString("tipo");
				aux[2] = rs1.getString("enunciado");
				aux[3] = rs1.getString("altA");
				aux[4] = rs1.getString("altB");
				aux[5] = rs1.getString("altC");
				aux[6] = rs1.getString("altD");
				aux[7] = rs1.getString("altE");
				aux[8] = rs1.getString("areaQuestao");
			}
